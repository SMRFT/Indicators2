...
const RecoveryWardRawData = ({ showHeading = true }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (form.checkValidity() === false) {
            ...
            try {
                ...
                const response = await fetch('http://127.0.0.1:8000/RecoverywardRawData/', {
                    method: 'POST',
                    headers: {
    ...
    return (
        <Container className="RawData">
            <div>
                {showHeading && <h2 className="text-center">RecoveryWard RawData</h2>}
