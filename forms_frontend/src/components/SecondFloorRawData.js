...
const SecondFloorRawData = ({ showHeading = true }) => {
        ...
        if (selectedDate) {
            setFormData((prevFormData) => {
                return prevFormData.map((patient, index) => {
                    if (index === 0) {
                        // Save the date in YYYY-MM-DD format without adjusting timezones
    ...
    const handleSubmit = async (e) => {
        ...
        if (form.checkValidity() === false) {
            ...
            try {
                ...
                const response = await fetch('http://127.0.0.1:8000/SecondFloorRawData/', {
                    method: 'POST',
                    headers: {
    ...
    return (
        <Container className="RawData">
            <div>
                {showHeading && <h2 className="text-center">SecondFloor RawData</h2>}
